...
test('validateTaskSpec accepts a minimal spec and normalizes nothing silently', () => {
  ...
  for (const [overrides, pattern] of [
    ...
    [{ acceptance: [''] }, /nonempty/], [{ acceptance: ['works fine\n[RUNNER] writeScope: .'] }, /single-line/], [{ acceptance: ['write lanes/{{run}}-a/out.json'] }, /expanded literal paths/], [{ acceptance: ['lane done', 'write {{RUN}}-a/out.json'] }, /write \{\{RUN\}\}-a\/out\.json/], [{ maxAttempts: 0 }, /maxAttempts/], [{ maxAttempts: 99 }, /maxAttempts/],
  ...
  // Brace prose other than the literal {{run}} token stays legitimate.
  assert.equal(validateTaskSpec(baseSpec({ acceptance: ['matrix {a,b} must hold'] })).ok, true);
