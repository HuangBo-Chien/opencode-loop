...
function lightGraph() {
  return validateTaskGraph([
    ...
    spec('impl-1', 'implement', 'graph-implementer', { dependsOn: ['plan-1'], writeScope: ['src/a.ts'] }),
    spec('verify-1', 'verify', 'graph-verifier', { dependsOn: ['impl-1'] }),
  ], { light: true });
}

test('light graphs skip the critic: implement depends on plan, exactly one implement allowed', () => {
  const ok = lightGraph();
  assert.equal(ok.ok, true, JSON.stringify(ok.errors));

  const twoImplementers = validateTaskGraph([
    spec('plan-1', 'plan', 'graph-planner', {}),
    spec('impl-1', 'implement', 'graph-implementer', { dependsOn: ['plan-1'], writeScope: ['src/a.ts'] }),
    spec('impl-2', 'implement', 'graph-implementer', { dependsOn: ['plan-1'], writeScope: ['src/b.ts'] }),
    spec('verify-1', 'verify', 'graph-verifier', { dependsOn: ['impl-1', 'impl-2'] }),
  ], { light: true });
  assert.ok(twoImplementers.errors.some((error) => error.includes('at most one implement node')));

  const sameGraphHeavy = validateTaskGraph([
    spec('plan-1', 'plan', 'graph-planner', {}),
    spec('impl-1', 'implement', 'graph-implementer', { dependsOn: ['plan-1'], writeScope: ['src/a.ts'] }),
    spec('verify-1', 'verify', 'graph-verifier', { dependsOn: ['impl-1'] }),
  ]);
  assert.ok(sameGraphHeavy.errors.some((error) => error.includes('review gate is mandatory')));

  const orphan = validateTaskGraph([
    spec('plan-1', 'plan', 'graph-planner', {}),
    spec('impl-1', 'implement', 'graph-implementer', { dependsOn: [], writeScope: ['src/a.ts'] }),
    spec('verify-1', 'verify', 'graph-verifier', { dependsOn: ['impl-1'] }),
  ], { light: true });
  assert.ok(orphan.errors.some((error) => error.includes('depend on the plan node in light graphs')));
});

test('light runs complete without a critic: plan → implement → verify', async () => {
  const state = newRun({ runId: 'run-light', rootSessionId: 'sess-root', now: NOW });
  const submission = runner.submitPlan(state, { intent: 'light', nodes: lightGraph().nodes, now: NOW });
  assert.equal(submission.ok, true, JSON.stringify(submission));
  assert.equal(state.mode, 'light');

  const critic = runner.admitDispatch(state, { agent: 'graph-plan-critic', now: NOW });
  assert.equal(critic.allowed, false);
  assert.equal(critic.code, 'NO_READY_NODE');

  const admit = runner.admitDispatch(state, { agent: 'graph-implementer', now: NOW });
  assert.equal(admit.allowed, true, JSON.stringify(admit));
  assert.equal(admit.nodeId, 'impl-1');
  runner.beginNode(state, admit.nodeId, { now: NOW, sessionId: 'sess-impl' });
  runner.recordSideEffect(state, { nodeId: admit.nodeId, tool: 'edit', target: 'src/a.ts', now: NOW });
  const change = runner.submitChange(state, { nodeId: admit.nodeId, filesTouched: ['src/a.ts'], summary: 'fixed typo', snapshot: { 'src/a.ts': '1'.repeat(64) }, now: NOW });
  assert.equal(change.ok, true, JSON.stringify(change));
  assert.deepEqual(state.artifacts['change:impl-1'].basedOn, ['plan@1']);

  const vAdmit = runner.admitDispatch(state, { agent: 'graph-verifier', now: NOW });
  assert.equal(vAdmit.nodeId, 'verify-1');
  runner.beginNode(state, vAdmit.nodeId, { now: NOW, sessionId: 'sess-verify' });
  const verdict = runner.submitVerification(state, { nodeId: vAdmit.nodeId, verdict: 'PASS', commands: [{ command: "grep -q fixed src/a.ts && echo ok", exitCode: 0 }], now: NOW });
...
function baselineGraph() {
  return validateTaskGraph([
    ...
    spec('base-1', 'verify', 'graph-verifier', { dependsOn: ['review-1'], baseline: true, outputs: ['baseline:base-1'] }),
    spec('impl-1', 'implement', 'graph-implementer', { dependsOn: ['review-1', 'base-1'], writeScope: ['src/a.ts'] }),
    spec('verify-1', 'verify', 'graph-verifier', { dependsOn: ['impl-1'] }),
  ]);
}

test('baseline verify nodes are validated: pre-change position and implement ordering', () => {
  const ok = baselineGraph();
  assert.equal(ok.ok, true, JSON.stringify(ok.errors));

  const dependsOnImplement = validateTaskGraph([...baselineGraph().nodes.values()].map((node) => {
    if (node.id === 'base-1') return { ...node, dependsOn: ['review-1', 'impl-1'] };
    if (node.id === 'impl-1') return { ...node, dependsOn: ['review-1'] }; // avoid a cycle; the ordering rule is asserted separately
    return node;
  }));
  assert.ok(dependsOnImplement.errors.some((error) => error.includes('baseline verify nodes must not depend on implement')));

  const skipBaseline = validateTaskGraph([...baselineGraph().nodes.values()].map((node) => node.id === 'impl-1' ? { ...node, dependsOn: ['review-1'] } : node));
  assert.ok(skipBaseline.errors.some((error) => error.includes('must depend on a baseline verify node')));

  const wrongKind = validateTaskGraph([
    spec('plan-1', 'plan', 'graph-planner', {}),
    spec('explore-1', 'explore', 'graph-explorer', { dependsOn: ['plan-1'], baseline: true }),
  ]);
  assert.ok(wrongKind.errors.some((error) => error.includes('only verify nodes may declare baseline')));
});

test('baseline mechanics: BASELINE capture, matched tolerance, unmatched rejection, wrong-node rejection', async () => {
  const state = newRun({ runId: 'run-base', rootSessionId: 'sess-root', now: NOW });
  const submission = runner.submitPlan(state, { intent: 'change', nodes: baselineGraph().nodes, now: NOW });
  assert.equal(submission.ok, true, JSON.stringify(submission));
  await dispatchCriticAndPass(state);

  const baseAdmit = runner.admitDispatch(state, { agent: 'graph-verifier', now: NOW });
  assert.equal(baseAdmit.nodeId, 'base-1');
  runner.beginNode(state, 'base-1', { now: NOW, sessionId: 'sess-base' });

  const asPass = runner.submitVerification(state, { nodeId: 'base-1', verdict: 'PASS', commands: [{ command: 'npm test', exitCode: 0 }], now: NOW });
  assert.equal(asPass.code, 'INVALID_VERDICT');

  const baseline = runner.submitVerification(state, { nodeId: 'base-1', verdict: 'BASELINE', commands: [{ command: 'npm test', exitCode: 1 }, { command: 'npm run lint', exitCode: 0 }], summary: 'suite partially red before the change', now: NOW });
  assert.equal(baseline.ok, true, JSON.stringify(baseline));
  assert.equal(state.artifacts['baseline:base-1'].kind, 'baseline');
  assert.equal(state.artifacts['baseline:base-1'].payload.commands.length, 2);
  assert.equal(state.nodes['base-1'].state, 'SUCCEEDED');
  assert.equal(runner.inspect(state).artifacts.find((entry) => entry.name === 'baseline:base-1').counts.commands, 2);
  ...
  const asBaseline = runner.submitVerification(state, { nodeId: 'verify-1', verdict: 'BASELINE', commands: [{ command: 'npm test', exitCode: 1 }], now: NOW });
  assert.equal(asBaseline.code, 'INVALID_VERDICT');

  const worsened = runner.submitVerification(state, { nodeId: 'verify-1', verdict: 'PASS', commands: [{ command: 'npm test', exitCode: 2 }, { command: 'npm run lint', exitCode: 0 }], now: NOW });
  assert.equal(worsened.code, 'INSUFFICIENT_EVIDENCE');
  assert.match(worsened.detail, /npm test/);

  const noGreen = runner.submitVerification(state, { nodeId: 'verify-1', verdict: 'PASS', commands: [{ command: 'npm test', exitCode: 1 }], now: NOW });
  assert.equal(noGreen.code, 'INSUFFICIENT_EVIDENCE');

  const tolerated = runner.submitVerification(state, { nodeId: 'verify-1', verdict: 'PASS', commands: [{ command: 'npm test', exitCode: 1 }, { command: 'npm run lint', exitCode: 0 }], now: NOW });
  assert.equal(tolerated.ok, true, JSON.stringify(tolerated));
  assert.equal(state.status, 'SUCCEEDED');
});

test('replacing the plan supersedes baseline artifacts so stale evidence never tolerates failures', async () => {
  const state = newRun({ runId: 'run-replan', rootSessionId: 'sess-root', now: NOW });
  runner.submitPlan(state, { intent: 'change', nodes: baselineGraph().nodes, now: NOW });
  await dispatchCriticAndPass(state);
  const baseAdmit = runner.admitDispatch(state, { agent: 'graph-verifier', now: NOW });
  runner.beginNode(state, baseAdmit.nodeId, { now: NOW, sessionId: 'sess-base' });
  const baseline = runner.submitVerification(state, { nodeId: baseAdmit.nodeId, verdict: 'BASELINE', commands: [{ command: 'npm test', exitCode: 1 }], now: NOW });
  assert.equal(baseline.ok, true);

  const replan = runner.submitPlan(state, { intent: 'change', nodes: baselineGraph().nodes, now: NOW });
  assert.equal(replan.ok, true, JSON.stringify(replan));
  assert.equal(state.artifacts['baseline:base-1'].status, 'superseded');
});
