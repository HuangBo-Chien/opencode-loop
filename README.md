...
- `graph_submit_findings` accepts `learnings`: durable patterns, pitfalls and principles, kept distinct from the traceable `evidence` trail. Learnings are injected into planner dispatch prompts and into the bounded `carryOver` digest a user reset creates, always with a revalidation mandate.
- `graph_inspect` surfaces per-artifact counts (`{ artifacts, probed, skipped }` for verifications, `{ risks }` for changes, `{ learnings }` for findings) so progress and evidence density stay observable without dumping payloads.
