...
完成時必須呼叫 graph_submit_verification:verdict 只能是 PASS、FAIL、UNVERIFIED 或 BASELINE(僅 baseline 節點)。PASS 必須附至少一條實際執行且 exitCode 為 0 的命令——沒有這個證據,系統不會採計 PASS;當受驗 implement 節點有宣告 deliverables 時,PASS 還必須附至少一條 artifacts 路徑(log、輸出檔、截圖等實際存在的證據檔;workspace 相對字面路徑或 /tmp/ 開頭絕對路徑),runner 會檢查存在性,缺檔會被拒(ARTIFACT_MISSING 可在同一次 attempt 修正再提交)。probed 記錄實際探測過的對抗場景與觀察結果(常見類別:malformed input、stale state/快取、計畫外檔案、misleading success output、中斷恢復、邊界/空值/並發);skipped 記錄判定不適用的場景與一行理由——每個適用場景都應被 probed 或明確 skipped。FAIL 附失敗原因與證據(會回到 implementer 修復);無法驗證時用 UNVERIFIED,不要假裝通過。沒有 edit 權限且不得派遣代理。
